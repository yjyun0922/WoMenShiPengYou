...
TEXT_LOCATION_LABEL = PATH_TO_FILES + 'list_category_img.txt'#'short_cat.txt'
TEXT_LOCATION_BBOX = PATH_TO_FILES + 'list_bbox.txt'#'short_bbox.txt'
...
#TEXT_LOCATION_CAT = PATH_TO_FILES + 'list_category_cloth.txt'
#TEXT_LOCATION_PART = PATH_TO_FILES + 'list_eval_partition.txt'
#NEW_TEXT_LOCATION_CAT = PATH_TO_FILES + 'new_cat_list.txt'
#save my address as a parameter to avoid having to retype it all the time 


def label_list_partitioned(text_location_label, text_location_part, text_location_cat): 
	...
	for counter in range(2, len(our_file), 1):  
		...
		label = get_cat(image_address, text_location_cat)
		#print(label)
	...
	for i in range(length):
		...
		example = tf.train.Example(features=tf.train.Features(feature={
			'image/height': int64_feature(h),
  			...
  			'image/encoded': bytes_feature(encoded_jpg), #'image/encoded': bytes_feature(image.tostring()),
...
#   images_addr, labels, files_name = label_list(TEXT_LOCATION_LABEL)
    training_label, testing_label, validating_label = label_list_partitioned(TEXT_LOCATION_LABEL, TEXT_LOCATION_PART, TEXT_LOCATION_CAT)
#   images, dims, bboxs = bbox_list(TEXT_LOCATION_BBOX)
    ...
    cats = ["Top", "Bottom", "Dress"]#cat_list(NEW_TEXT_LOCATION_CAT)
